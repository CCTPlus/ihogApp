...
class HogPersistenceControllerTests {
  ...
  @Test("Verifies view context configuration")
  ...
  @Test("Verifies in-memory store configuration")
  func testInMemoryStore() throws {
    let controller = HogPersistenceController(inMemory: true)
    let description = controller.container.persistentStoreDescriptions.first

    #expect(description?.url?.absoluteString == "file:///dev/null")
    #expect(controller.container.viewContext.automaticallyMergesChangesFromParent)
  }
}
