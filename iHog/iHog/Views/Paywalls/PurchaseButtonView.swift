...
struct PurchaseButtonView: View {
    ...
    var body: some View {
        VStack {
                ...
                HStack(alignment: .top) {
                    VStack(alignment: .leading) {
                        if package?.isSubscription == true {
                            Text(package?.storeProduct.subscriptionPeriod?.durationAdverb.localizedCapitalized ?? "")
                                .font(.headline)
                            if package?.hasFreeTrial == true {
                                Text("First \(package?.discountTerms ?? freeTrial) are free,\nthen \(price) \(units)")
                                    .lineLimit(nil)
            ...
            .background(RoundedRectangle(cornerRadius: BASE_CORNER_RADIUS).fill(backgroundColor.opacity(0.2)))
            .background(RoundedRectangle(cornerRadius: BASE_CORNER_RADIUS)
