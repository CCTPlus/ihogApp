...
//  iHog
//
//  Created by Jay Wilson on 1/26/22.
//
...
struct UserFeedbackView: View {
  ...
  let appVersion: String =
    Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "2022"
  ...
  private func addIssueToGithub() {
    ...
    Octokit(config)
      .postIssue(
        ...
        body: issueBody,
        assignee: "Jaywilson",
        labels: [selectedFeedbackType.rawValue]
      ) { response in
        switch response {
          case .success:
            buttonState = .sent
            toast.animateIn(text: "Feedback submitted", color: Color.green)
            userState.resetNavigation()
          case .failure(let error):
            print(error)
            errorText = error.localizedDescription
            buttonState = .error
            toast.animateIn(text: errorText, color: .red)
        }
      }
  }
}
